...
`strings.ContainsAny(first, ".:")` 패턴으로 레지스트리 여부를 판별했다. 이는 Docker CLI가 사용하는 휴리스틱과 동일���다 — 도메인 이름은 반드시 `.`을 포함하고, `localhost:5000` 같은 경우는 `:`를 포함하기 때문.
...
이 차이가 backward compatibility에 중요하다. 기존에 `{"command":["sleep","10"]}` 형태로 직렬화되던 `KernelSpec`이 `image` 필드 없이 그대��� 유지되어야 하기 때문.
...
`go get github.com/docker/docker`를 실행하면 `v28.5.2+incompatible`처럼 `+incompatible` 태그가 붙는다. 이는 Docker SDK가 아직 Go 모듈 체계(`go.mod`)를 완전�� 채택하지 않았기 때문이다.
