...
| `KernelStatusType`을 string named type으로 구현 | JSON에 `"running"` 등 읽기 쉬운 문자열 출력, stringer 불필요 | iota int — JSON에 정수 출력되어 의미 불명, 코드 생성 도구 필요 |
...
- [ ] string named type enum에 검증 함수 추가 패턴 — `ParseKernelStatusType(s string)` 등
