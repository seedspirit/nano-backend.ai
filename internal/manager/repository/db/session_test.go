...
func TestMigrateIsIdempotent(t *testing.T) {
	...
	repo := newTestSessionRepository(t, ctx)

	if err := Migrate(ctx, repo.db); err != nil {
		t.Fatalf("second migration terminated: %v", err)
	}
	...
	if err := repo.db.GetContext(ctx, &categoryCount, `SELECT COUNT(*) FROM preset_categories`); err != nil {
		t.Fatalf("terminated to inspect preset categories: %v", err)
	}
	if categoryCount != 3 {
	...
	if err := repo.db.GetContext(ctx, &presetCount, `SELECT COUNT(*) FROM presets`); err != nil {
		t.Fatalf("terminated to inspect presets: %v", err)
	}
	if presetCount != 2 {
...
func TestListProjectSessionsReturnsMostRecentSessionsWithinLimit(t *testing.T) {
	...
	if got[0].ID != newSessionID {
		t.Fatalf("got first session id %s, want %s", got[0].ID, newSessionID)
	}
	if got[1].ID != middleSessionID {
		t.Fatalf("got second session id %s, want %s", got[1].ID, middleSessionID)
	}
	if got[0].Lifecycle.Status != session.Pending {
		t.Fatalf("got status %s, want %s", got[0].Lifecycle.Status, session.Pending)
	}
}
...
func TestListProjectSessionsReturnsEmptyForProjectWithoutSessions(t *testing.T) {
	...
	got, err := fixture.repo.ListProjectSessions(fixture.ctx, projectID, 20)
	if err != nil {
		t.Fatalf("list project sessions: %v", err)
	}
	if got == nil {
		t.Fatal("got nil sessions, want empty slice")
	}
	if len(got) != 0 {
...
func TestListProjectSessionsReturnsNotFoundForMissingProject(t *testing.T) {
	fixture := newSessionRepositoryFixture(t)
	projectID := uuid.New()

	_, err := fixture.repo.ListProjectSessions(fixture.ctx, projectID, 20)
	if !errors.Is(err, errordef.ErrNotFound) {
		t.Fatalf("got err %v, want ErrNotFound", err)
	}
}

func newTestSessionRepository(t *testing.T, ctx context.Context) *SessionRepository {
	t.Helper()
	repo, err := NewSessionRepository(ctx, Args{
	...
	t.Cleanup(func() {
		if err := repo.Close(); err != nil {
			t.Errorf("close session repository: %v", err)
		}
	})
	return repo
}
...
func newSessionRepositoryFixture(t *testing.T) *sessionRepositoryFixture {
	t.Helper()
	ctx := context.Background()
...
func (f *sessionRepositoryFixture) givenProject() uuid.UUID {
	f.t.Helper()
	id := uuid.New()
...
func sampleSpec(projectID uuid.UUID) spec.Spec {
	specID := uuid.New()
	trainerID := sessionspecpreset.PresetAxolotlLoRASFT
	return spec.Spec{
		ID:           specID,
		ProjectID:    projectID,
		Name:         "submission-1",
