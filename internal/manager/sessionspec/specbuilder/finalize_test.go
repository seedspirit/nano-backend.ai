...
func TestFinalizeSessionSpecMergesDefaultsAndParameters(t *testing.T) {
	sessionDraft := sampleDraft()
	trainerPreset := sessionspecpreset.AxolotlLoRASFT()

	finalized := FinalizeSessionSpec(Candidate{Draft: &sessionDraft, Presets: preset.Presets{Trainer: &trainerPreset}})
...
func TestFinalizeSessionSpecDoesNotMutatePresetDefaults(t *testing.T) {
	sessionDraft := sampleDraft()
	...
	trainerPreset := testPreset{
		id:       sessionspecpreset.PresetAxolotlLoRASFT,
		defaults: defaults,
	}
...
func TestFinalizeSessionSpecAppliesPresetDataAndKeepsPresetRefs(t *testing.T) {
	sessionDraft := sampleDraft()
	trainerPreset := sessionspecpreset.AxolotlLoRASFT()
	resourcePreset := optionPreset{
		...
		resources: preset.ResourceOptions{
			CPU:     session.CPUOptions{Cores: 8},
			GPU:     session.GPUOptions{Count: 4},
			Memory:  session.MemoryOptions{LimitBytes: 68719476736},
			Timeout: session.TimeoutOptions{DurationSeconds: 28800},
		},
	}
	outputPreset := testPreset{id: uuid.MustParse("7f973e96-5bed-47d4-805e-b4d99c8638cf")}

	finalized := FinalizeSessionSpec(Candidate{Draft: &sessionDraft, Presets: preset.Presets{
	...
	if finalized.PresetRefs.Trainer == nil || *finalized.PresetRefs.Trainer != sessionspecpreset.PresetAxolotlLoRASFT {
		t.Fatalf("got trainer preset ref %v, want %s", finalized.PresetRefs.Trainer, sessionspecpreset.PresetAxolotlLoRASFT)
	}
...
func sampleDraft() draft.Draft {
	return draft.Draft{
		...
		ResourceOptions: draft.ResourceOptionsReq{
			...
			Memory:  session.MemoryOptions{LimitBytes: 34359738368},
			Timeout: session.TimeoutOptions{DurationSeconds: 14400},
		},
